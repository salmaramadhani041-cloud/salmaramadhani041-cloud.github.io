...
<div class="page-wrap">
  ...
  <div class="top-bar">
    <h1>Daftar Perintilan</h1>
    <a href="tambah.php" class="btn btn-primary">＋ Tambah Barang</a>
  </div>

  <?php
    $totalBarang = (int) $pdo->query("SELECT COUNT(*) FROM barang")->fetchColumn();
    $totalStok   = (int) ($pdo->query("SELECT SUM(stok) FROM barang")->fetchColumn() ?? 0);
    $totalNilai  = (float) ($pdo->query("SELECT SUM(harga * stok) FROM barang")->fetchColumn() ?? 0);
  ?>
  <div class="stats-row">
    <div class="stat-card">
    ...
    <div class="stat-card">
      ...
      <div class="val">Rp <?= number_format($totalNilai) ?></div>
    </div>
  </div>
  ...
  <div class="table-card">
    <table>
      <thead>
        <tr>
          <th>ID</th><th>Nama Barang</th><th>Kategori</th><th>Stok</th>
          <th>Harga</th><th>Lokasi Rak</th><th>Aksi</th>
        </tr>
      </thead>
      <tbody>
      <?php
